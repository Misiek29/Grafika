...
void GenerateTextures()
{
	...
	GLboolean error = load_targa("grass_prospective.tga", width, height, format, type, pixels);

	// błąd odczytu pliku
	if (error == GL_FALSE)
	{
		printf("Niepoprawny odczyt pliku grass_prospective.tga");
		exit(0);
	}
	...
	error = load_targa("maple_tree_small.tga", width, height, format, type, pixels);

	// błąd odczytu pliku
	if (error == GL_FALSE)
	{
		printf("Niepoprawny odczyt pliku shrub_medium_size.tga");
		exit(0);
	}
